...
class Game
  ...
  def analyze_turn(turn)
    ...
    # Hier wird der Code geklont, damit in der each_with_index schleife die bereits analysierten Ziffern aus dem Array entfernt werden
    # Nur so stimmt die Auswertung der white hits  
    analyze_clone = @code.clone()
    
    turn.code.each_with_index do | value, index|
      if value == @code[index]
        black_hits += 1
      else
        if analyze_clone.include?(value)
          white_hits += 1
        end
      end
      analyze_clone.delete_at(index)   
    end
    
    # Zrueckschreiben des Ergebnisses des Zuges
